...
namespace GeocachingToolbox.GeocachingCom
{
    public class GCClient : Client
    {
        ...
        //TODO : get date format from preferences
        ...
        public override IEnumerable<T> GetFoundGeocaches<T>()
        {
            ...
            foreach (var row in rows)
            {
                ...
                var date = DateTime.ParseExact(element.TextContent.Trim(), DateFormat, CultureInfo.CurrentCulture);
        ...
        public override void GetGeocacheDetails<T>(T geocache)
        {
            ...
            if (string.IsNullOrEmpty(gcGeocache.Code) == false)
            {
                html = _connector.GetPage("geocache/" + gcGeocache.Code);
            }
        ...
        private void ParseHiddenDate(GCGeocache gcGeocache, IHtmlDocument doc)
        {
            ...
            gcGeocache.DateHidden = DateTime.ParseExact(date, DateFormat, CultureInfo.CurrentCulture);
        }
        ...
        private static void ParseName(GCGeocache gcGeocache, IHtmlDocument doc)
        {
            if (gcGeocache.IsPremium)
            {
                var name = doc.QuerySelector("div[id=ctl00_divContentMain] h2").TextContent; //DocumentNode.SelectSingleNode("//div[@id=\"ctl00_divContentMain\"]/h2")
                                                                                             //.InnerText;
        ...
        private static void ParseType(GCGeocache gcGeocache, IHtmlDocument doc)
        {
            ...
            {
                type = doc.QuerySelector("div[id=cacheDetails] p[class=cacheImage] a img")
        ...
        private static void ParseStatus(GCGeocache geocache, IHtmlDocument doc)
        {
            ...
            if (statusListItems == null || statusListItems.Length == 0)
            {
